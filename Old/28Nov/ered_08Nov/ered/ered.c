...
* TEAM : Mitul Panchal
  ...
  Description: This module implements the queue APIs
		to provide new queueing discipline
		in linux tc
**************************************************/
...
static int showEmerStats(struct seq_file *m, void *v)
{
	...
	for(i=0; i<MAX_COLORS; i++)
	{
		if(i==1)
		   continue;
		seq_printf(m, "color:%d, total = %d, min = %d, pdrop = %d, fdrop = %d qlen=%d qavg=%d|", i, ered_stats[i].total, ered_stats[i].min, ered_stats[i].pdrop, ered_stats[i].fdrop, ered_stats[i].colorOccupancy, ered_stats[i].qavg);
		seq_printf(m, "etotal = %d, emin = %d, emark = %d,epdrop=%d,efdrop = %d\n", ered_stats[i].etotal, ered_stats[i].emin, ered_stats[i].emark, ered_stats[i].epdrop,ered_stats[i].efdrop);
	}
    	return 0;
}
